...
# ==============================================================================
# 3. 物理たわみ補正レイヤー (方法 2: 2 次元分離 2 次多項式モデル)
# ==============================================================================
def calculate_sag_compensation(r_m: float, theta_rad: float) -> float:
    """
    アームの自重およびリンク弾性による手先の沈み込み量 Δz [m] を推計する。
    
    【数式モデル (実機回帰分析: R^2 = 0.9689, RMSE = 4.82mm)】
      Δz_mm = 871.43 * r^2 - 172.05 * r + 8.11 * θ^2 + 29.89  [mm]
    
    引数:
        r_m: 旋回中心からの水平リーチ長 [m]
        theta_rad: 台座旋回角度 [rad] (正面 0 rad, 左右対称評価)
    戻り値:
        補正持上げ量 Δz [m] (目標 z 座標に加算して使用)
    """
    # 測定有効レンジ [0.15m, 0.40m] にクランプ
    r_clamped = max(0.15, min(0.40, r_m))

    # 2次元多項式により沈み込みミリ数を算出
    delta_z_mm = (
        871.42857 * (r_clamped ** 2)
        - 172.04762 * r_clamped
        + 8.10569 * (theta_rad ** 2)
        + 29.89048
    )
