...
# 1. 再生設定・通信設定
# ==========================================
PLAYBACK_FILENAME = "motion_20260816_215707.csv"  # ファイル名  
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  
PLAYBACK_FILE = os.path.join(BASE_DIR, "motions", PLAYBACK_FILENAME)  # 再生対象CSVパス  
REPEAT_COUNT = 2                   # 再生回数  

# 安全動作の時間設定
HOME_RETURN_DURATION = 2.5         # 規定初期位置への復帰にかける秒数（より滑らかに）
...
def main():
    ...
    try:
        ...
        for loop_idx in range(REPEAT_COUNT):  
            print(f"\n========================================")
            print(f" 🔄 周回開始 [{loop_idx + 1} / {REPEAT_COUNT} 周目]")
            print(f"========================================")

            # 1. 周回の合間（2周目以降）：直前の終了姿勢からHomeへゆっくり戻る
            if loop_idx > 0:
                print(f"🏠 規定の初期位置へ復帰中 ({HOME_RETURN_DURATION}秒)...")
                smooth_move_follower(ser_follower, current_state, home_positions, duration=HOME_RETURN_DURATION)
                current_state = dict(home_positions)
            ...
            # 2. Home ➔ モーション開始点へゆっくりアプローチ
            print(f"🎯 モーション開始姿勢へアプローチ中 ({MOTION_START_TRANSITION}秒)...")
        ...
        # 4. 全周回完了後の最終復帰（最終停止位置 ➔ Home）
        print(f"\n🏠 すべての再生終了。規定の初期位置へゆっくり復帰中 ({HOME_RETURN_DURATION}秒)...")
        smooth_move_follower(ser_follower, current_state, home_positions, duration=HOME_RETURN_DURATION)
        time.sleep(0.5)

        print("✅ すべてのシークエンスが安全に完了しました。")
